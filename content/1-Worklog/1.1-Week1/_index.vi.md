...
## 1. Mục tiêu tuần
Bắt đầu hành trình First Cloud AI Journey (FCAJ), mình đặt ra các mục tiêu để làm quen với môi trường Cloud của AWS:
...
Dưới đây là nhật ký công việc chi tiết của mình trong tuần đầu tiên:
...
## 3. Kết quả đạt được
Qua tuần đầu thực hành và tự học, mình đã đúc kết được các bài học kinh nghiệm rất thực tế dưới đây:

* **Quản lý chi phí và ngân sách thực hành (FinOps):**
  + Mình đã hoàn thành 5 nhiệm vụ cơ bản trên Console (tạo máy ảo EC2, gọi API Amazon Bedrock qua Claude 3, cấu hình cảnh báo Budgets, chạy Serverless Lambda và DB RDS Aurora) để kích hoạt đủ $200 AWS Credit.
  + Đúc kết được thói quen FinOps quan trọng là bật cảnh báo AWS Budgets sớm (gửi email cho mình ngay khi chi phí vượt quá $1) và tiến hành xóa/terminate tài nguyên (EC2, RDS Aurora) ngay sau khi làm lab xong để tránh phát sinh chi phí ngoài ý muốn.
  
  ![Thiết lập AWS Budgets thành công](/images/1-Worklog/Week1/budget-success.png)
  
* **Tư duy bảo mật hệ thống (IAM):**
  + Tự mình thiết lập MFA bảo vệ tài khoản Root và thực hành tạo IAM User, Group để quản lý quyền hạn của từng thành viên trong nhóm.
...
* **Phương pháp phát triển hướng đặc tả (AI Kiro):**
  + Làm quen với phương pháp **Spec-driven development**. Thay vì ngồi code tay từng dòng, mình học cách mô tả thiết kế và nghiệp vụ hệ thống qua file cấu hình chuẩn, sau đó để AI Kiro tự động sinh mã nguồn và hạ tầng tương ứng.
